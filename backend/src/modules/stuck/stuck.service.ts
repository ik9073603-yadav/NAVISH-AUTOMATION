...
// Everything currently stuck/overdue/escalated across every module, in one
// place — the single source of truth for "stuck" so the Stuck tab and the
// Health Score's stuck-load component can never diverge. Unplanned FMS
// stages (plannedMins == null) never appear here, same as always.
export async function getStuckItems(orgId: string): Promise<StuckItem[]> {
  const now = new Date();
